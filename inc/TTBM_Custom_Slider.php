...
*/
	...
	if (!class_exists('TTBM_Custom_Slider')) {
		class TTBM_Custom_Slider {
			...
			public function slider($post_id, $image_ids) {
				if (is_array($image_ids) && sizeof($image_ids) > 0) {
					$showcase_position = TTBM_Global_Function::get_slider_settings('showcase_position', 'right');
                    ...
                    <div class="superSlider placeholder_area fdColumn">
								...
								if ($slider_style == 'style_2') {
                                    ...
                                    <div class="abTopLeft">
                                        <button type="button" class="_dButton_bgWhite_textDefault" data-target-popup="superSlider" data-slide-index="1">
											<?php echo esc_html__('View All', 'tour-booking-manager') . ' ' . esc_html(sizeof($image_ids)) . ' ' . esc_html__('Images', 'tour-booking-manager'); ?>
                                        </button>
                                    </div>
									<?php
								}
						...
						<?php
							$slider_indicator = TTBM_Global_Function::get_slider_settings('indicator_visible', 'on');
							$icon = TTBM_Global_Function::get_slider_settings('indicator_type', 'icon');
			...
			public function slider_showcase($image_ids) {
				$showcase = TTBM_Global_Function::get_slider_settings('showcase_visible', 'on');
			...
			public function slider_popup($post_id, $image_ids) {
				if (is_array($image_ids) && sizeof($image_ids) > 0) {
					$popup_icon_indicator = TTBM_Global_Function::get_slider_settings('popup_icon_indicator', 'on');
					?>
                    <div class="sliderPopup" data-popup="superSlider">
                        <div class="superSlider">
                            ...
                            <div class="popupFooter">
								<?php
									$indicator = TTBM_Global_Function::get_slider_settings('popup_image_indicator', 'on');
									if ($indicator == 'on') {
										$this->image_indicator($image_ids);
									}
								?>
			...
			public function get_slider_ids($post_id, $key) {
				...
				$image_ids = TTBM_Global_Function::get_post_info($post_id, $key, array());
				if ($thumb_id) {
