...
if (!class_exists('TTBM_Hidden_Hotel_Product')) {
    class TTBM_Hidden_Hotel_Product{
        ...
        public function create_hidden_wc_product_on_publish($post_id, $post) {
            if ($post->post_type == 'ttbm_hotel' && $post->post_status == 'publish' && empty(MP_Global_Function::get_post_info($post_id, 'check_if_run_once'))) {
                $new_post = array(
            ...
            if (is_product()) {
                ...
                if (is_object($visibility)) {
                    if ($visibility[0]->name == 'exclude-from-catalog') {
                        $check_event_hidden = MP_Global_Function::get_post_info($post_id, 'link_ttbm_id', 0);
        ...
        public function get_all_hidden_product_id() {
            $product_id = [];
            $query = MP_Global_Function::query_post_type('ttbm_hotel');
