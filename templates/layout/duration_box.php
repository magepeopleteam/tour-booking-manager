<?php
	...
	$day       = TTBM_Function::get_post_info( $ttbm_post_id, 'ttbm_travel_duration' );
	$night     = TTBM_Function::get_post_info( $ttbm_post_id, 'ttbm_travel_duration_night' );
	$duration_type=TTBM_Function::get_post_info( $ttbm_post_id, 'ttbm_travel_duration_type', 'day' );
	$tour_type = TTBM_Function::get_tour_type( $ttbm_post_id );
	$count     = $count ?? 0;
	if ( ( $day || $night ) && $tour_type == 'general' && TTBM_Function::get_post_info( $ttbm_post_id, 'ttbm_display_duration', 'on' ) != 'off' ) {
		?>
			...
			<h6>
				...
				<strong>
					<?php
						...
						if ( TTBM_Function::get_post_info( $ttbm_post_id, 'ttbm_display_duration_night', 'off' ) != 'off' ) {
							if ( $night && $night > 1 ) {
