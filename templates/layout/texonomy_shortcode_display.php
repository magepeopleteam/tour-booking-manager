...
if( $carousel == 'no' ){
    if( $load_more_button === 'yes' ){
        $load_more_class = 'ttbm_load_'.$taxonomy_type.'';
        $display = 'none';
    }else{
        $load_more_class = '';
        $display = '';
...
<!--            --><?php //do_action( 'ttbm_section_title', 'ttbm_string_related_tour', esc_html__( ucfirst( $taxonomy_type ), 'tour-booking-manager' ) ); ?>
            ...
            <div class="placeholder_area flexWrap" id="ttbm_<?php echo esc_attr( $taxonomy_type )?>_placeholder_area">
                <?php if ( $carousel == 'no' ) {?>
                    <input type="hidden" id="ttbm_load_more_<?php echo esc_attr( $taxonomy_type );?>_number" value="<?php echo esc_attr( $num_of_display )?>">
                <?php }?>
                <?php } else {?>
                <div class="ttbm_widget_content _mZero  <?php echo esc_attr( $related_tour_count >$num_of_tour?'owl-theme owl-carousel':$div_class); ?>" data-show ="<?php echo esc_attr($num_of_display); ?>">
                    <?php }?>
                    <?php
                    include(TTBM_Function::template_path('list/taxonomy_lists.php'));
                    ?>

                </div>
                <?php  if( $load_more_button == 'yes' && $carousel == 'no' &&  $related_tour_count > $num_of_display ){?>
                    <div class="ttbm_attraction_load_more_holder"><div class="ttbm_load_more_text" id="ttbm_<?php echo esc_attr( $taxonomy_type );?>_load_more_text">Load More...</div></div>
                <?php }?>
            </div>
